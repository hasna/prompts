...
function useJson(program: Command, opts: { json?: boolean }): boolean {
  ...
  storage
    ...
    .action(async (opts: { tables?: string; json?: boolean }) => {
      try {
        ...
        if (useJson(program, opts)) printJson(results)
        else printResults(results)
      } catch (error) {
        if (useJson(program, opts)) printJson({ error: error instanceof Error ? error.message : String(error) })
  ...
  storage
    ...
    .action(async (opts: { connectionString?: string; json?: boolean }) => {
      try {
        ...
        const result = await applyPgMigrations(opts.connectionString || getStorageConnectionString("prompts"))
        if (useJson(program, opts)) {
          printJson(result)
          return
        }
    ...
    .action((message: string, opts: { email?: string; category?: string; json?: boolean }) => {
      try {
        ...
        db.run(
          ...
          [message, opts.email || null, opts.category || "general", "prompts"]
        )
        if (useJson(program, opts)) printJson({ saved: true })
...
export function registerStorageCommands(program: Command): void {
  installStorageSubcommands(program.command("storage").description("Manage prompts local/remote storage sync"), program)
}
